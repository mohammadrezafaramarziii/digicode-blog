...
async function PostList() {
    ...
    return (
        <div className="w-full grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 xl:grid-cols-3 gap-6">
            {posts.map((post) => (
                <div key={post._id} className="w-full bg-primary-800 rounded-3xl">
                    ...
                    <div className="w-full p-4">
                        ...
                        <Link href={`/blogs/${post.slug}`}>
                            <h2 className="text-secondary-900 text-lg font-semibold mb-1 hover:text-primary-900 duration-100">
                                {post.title}
                            </h2>
                        </Link>
